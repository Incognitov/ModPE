...
function createSortedInventory() {
    for (var i = 9; i <= 44; i++) {
        if (Player.getInventorySlot(i) != 0) {
            ...
            if (rmq == 0) {
                clientMessage("[Inventory Tweaks] §o§7 I found some item I didn't know where to sort, so I put them in the end.");
                rmq++
            }
        }
    }
    for (var i = 0; i <= inventory.length - 1; i++) {
        if (i <= 35) {
            ...
            if (rmq < 2) {
                clientMessage("[Inventory Tweaks] §o§7 There happened to be too many items for your inventory, so I dropped them aside.");
                rmq = 2;
            }
        }
    }
...
function createSortedChest(x, y, z, le) {
    ...
    for (var i = 0; i <= chest.length - 1; i++) {
        if (i <= le) {
            ...
            if (rmq < 2) {
                clientMessage("[Inventory Tweaks] §o§7 There happened to be too many items for this chest, so I dropped them aside.");
                rmq = 2;
            }
        }
    }
